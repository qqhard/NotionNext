...
async function convertNotionToSiteData(SITE_DATABASE_PAGE_ID, from, pageRecordMap) {
  ...
  const { ids: pageIds, pageSortCount } = getAllPageIds(
    collectionQuery,
    collectionId,
    collectionView,
    viewIds,
  ...
  // Sorting: prefer Notion view order (page_sort), fallback to priority desc + date desc
  const pageSortSet = new Set(pageIds.slice(0, pageSortCount))
  const sortByPriorityDate = (a, b) => {
    const aPri = a?.priority != null && a.priority !== '' ? Number(a.priority) : -Infinity
    const bPri = b?.priority != null && b.priority !== '' ? Number(b.priority) : -Infinity
    if (aPri !== bPri) return bPri - aPri
    return (b?.publishDate || 0) - (a?.publishDate || 0)
  }

  if (pageSortCount > 0) {
    // Split: pages from page_sort keep view order, the rest sorted and appended
    const viewOrderPages = []
    const supplementPages = []
    for (const page of allPages) {
      if (pageSortSet.has(page.id)) {
        viewOrderPages.push(page)
      } else {
        supplementPages.push(page)
      }
    }
    supplementPages.sort(sortByPriorityDate)
    allPages.length = 0
    allPages.push(...viewOrderPages, ...supplementPages)
  } else {
    allPages.sort(sortByPriorityDate)
  }
